...
Revision ID: e375c46f2117
Revises: 5be21d8f4a97
Create Date: 2025-08-28 21:39:41.042682
...
revision: str = 'e375c46f2117'
down_revision: Union[str, None] = '5be21d8f4a97'
...
def upgrade() -> None:
    ...
    op.alter_column('operaciones', 'tipo_operacion',
    ...
    op.create_foreign_key(None, 'operaciones', 'task_list', ['tipo_operacion'], ['task_name'])
    ...
    op.add_column('task_list', sa.Column('task_key', sa.String(), nullable=True))
    op.execute(
        """
        UPDATE task_list
        SET task_key = CASE 
            WHEN task_type = 'vineyard' THEN 'V' || task_list_id::TEXT
            WHEN task_type = 'winery' THEN 'W' || task_list_id::TEXT
            ELSE 'Z' || task_list_id::TEXT
        END
        """
    )
...
def downgrade() -> None:
    ...
    op.alter_column('task_list', 'task_type',
               existing_type=sa.VARCHAR(length=255),
               nullable=False)
    op.drop_column('task_list', 'task_key')
